...
const checkTrainings = async() => {
    try {
        ...
        trainings.map(async training => {
            const trainDate = new Date(training.Date[0][0]).getTime()
            if (trainDate < TodayDate) {
                await Training.updateOne({ '_id': training._id }, { $set: { state: "expired" } })
                await TrainerNotifs.updateOne({ 'course': training._id }, { $set: { StatusMandate: "expired" } })
            } else {
...
router.post("/CreateTraining", async(req, res) => {
    ...
    try {
        ...
        await new Training(req.body).save();

        console.log("saved")


        const training = await Training.findOne({ Title: req.body.Title });

        await Category.updateOne({ Title: req.body.Category }, { $push: { Trainings: training._id } })


        return res.status(201).send({ message: "Training created successfully" });
...
router.post("/deleteTraining", authenticateToken, async(req, res) => {
    ...
    try {
        ...
        //console.log(admin)
        const training = await Training.findOne({ _id: ObjectId(req.body.idTraining) });

        if (admin) {
            await Room.delete({ courseId: training._id })
            await TrainerNotifs.delete({ 'course': training._id })
            await Category.updateOne({ Title: training.Category }, { $pull: { Trainings: training._id } })

            if (training.Trainer !== "") {
                await Trainer.updateOne({ _id: training.Trainer }, { $pull: { Trainings: req.body.idTraining } })
            }
            await Candidat.updateOne({ _id: { $in: training.enrolled } }, { $pull: { TrainingsPaid: req.body.idTraining }, $pull: { cartTrainings: req.body.idTraining } })
