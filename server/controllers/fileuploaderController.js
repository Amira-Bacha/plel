...
const singleFileUploadWithName = async (req, res, next) => {
    ...
    try{
        ...
        if(course){
            await Course.updateOne({_id:course._id},{$set:{Thumbnail: file}})
            
        }

        if(training){
            await Training.updateOne({_id:training._id},{$set:{Thumbnail: file}})
...
const multipleFileUpload = async (req, res, next) => {
    ...
    try{
        ...
        if(existFiles){
            await MultipleFile.updateOne({title : name},{ $push: { files: filesArray } })
        }else{
        ...
        if(course){
            ...
            await Course.updateOne({_id:course._id},{$set:{Videos: course.Videos}})
        ...
        if(training){
            ...
            await Training.updateOne({_id:training._id},{$set:{Ressources: filesArray}})
...
const multipleFilesUploadWithName = async (req, res, next) => {
    ...
    try{
        ...
        if(course){
            console.log("course: ",course)
            if(type==="Ressources"){
                await Course.updateOne({_id:course._id},{$set:{Ressources: filesArray}})
            }
            if(type==="Videos"){
                await Course.updateOne({_id:course._id},{$set:{Videos: filesArray}})
            }
        }
